/**
 * Фикстуры публичной страницы самообслуживания клиента (прототип, PM-синк
 * 07.08.2026 + дейли 07.08.2026).
 *
 ...
 * независимых счёта (`01-meta/04-system-context` → «Финансовая модель»).
 * Строки «Разом» в payload нет намеренно: её неоткуда взять.
 *
...
export interface PublicProfileFixture {
   ...
   * что они есть, но не их состав и суммы. Компромисс между решением дейли
   * «показуємо оренди всіх профілів» и утечкой корпоративных сумм тому, кто
   * держит телефон. Помечен как предложение — см. README.
   */
  concealed: boolean
}
