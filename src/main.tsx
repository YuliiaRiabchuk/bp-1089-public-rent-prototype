...
/**
 * Точка входу standalone-прототипу.
 *
 * В основній системі сторінка живе на роутах `/o` і `/o/$token` (TanStack
 * Router). Тут роутера немає — окремий проєкт заради одного екрана його не
 * вартий, тому набір демо-даних читається прямо з адреси.
 *
 * Підтримані форми (усі дають один результат), шлях рахується від `BASE_URL` —
 * на GitHub Pages сайт лежить у `/<repo>/`, а не в корені:
 *   .../           → demo
 *   .../o          → demo
 *   .../o/holiday  → holiday   ← так само, як в основній системі
 *   .../holiday    → holiday
