/**
 * Клиент публичной страницы аренды. Отдельный от `backendClient` слой: у
 * страницы нет сессии сотрудника и нет доступа к `/api/v1/*` — только к
 * своему `/api/public/*`.
 *
 * Модель входа — по НОМЕРУ, а не по ссылке (решение дейли 07.08.2026). Ссылка
 * не секрет и ничего не открывает: секрет — код, который приходит на номер.
...
/** Признак «счёт оплачен» приходит из 1С — страница его только читает. */
